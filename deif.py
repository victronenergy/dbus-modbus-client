...
class Reg_DEIF_ident(Reg_text):
    """ The DEIF controller identifies itself by a 9 char model name string,
...
class Reg_DEIF_status(Reg, int):
    ...
    def decode(self, values):
        val = values[0]
        return self.update(
            # Register 1018, bit 6: Engine running => 8=Running else 0=Stopped
            8 if val & 1 << 6 else 0
        )
...
class reg_DEIF_RemoteStartMode(Reg, int):
    ...
    def decode(self, values):
        return self.update(
            1 if (values[0] & 1 << 3) else 0 # AUTO
        )


class Reg_DEIF_unit_agnostic(Reg_s16):
...
class Reg_DEIF_temp(Reg_DEIF_unit_agnostic):
...
class Reg_DEIF_pressure(Reg_DEIF_unit_agnostic):
...
class DEIF_Generator(device.CustomName, device.ErrorId, device.Genset):
    ...
    def device_init(self):
        ...
        self.data_regs = [
            ...
            Reg_packed(1000, 16, bits=1, items=16, lsb=True,
                       onchange=self.alarm_changed),
            Reg_packed(1057, 1, bits=1, items=10, lsb=True,
                       onchange=self.alarm_changed),
        ]
    ...
    def _start_genset(self, _, value):
        ...
        # Auto start/stop
        if value: self.write_modbus(6, [(1 | 1 << 11)])
        else:     self.write_modbus(6, [(1 | 0 << 11)])
